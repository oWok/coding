...
//    println(thisisFunction2(3, null))
    ...
    joinToGuild(starGuild, "a", 50)
    joinToGuild(starGuild, "son", 190, 3)
    joinToGuild(starGuild, "ㅇ", 190, 0, -65f)
    joinToGuild(starGuild, "원종우", 189, 6, 150f)
    joinToGuild(starGuild, "20ㅁ7원종우", 0, 7, 150f)
...
fun joinToGuild(guild: MutableMap<String, String>, nick: String?, level: Int = 10, type: Int = 0, stat: Float = 5f) {
    val playerTypes = arrayOf("직원", "검사", "마법사", "궁수", "성직자", "도적", "ㅇ예정자", "ㅇ자")
    val name = nick ?: "이름없음"
    val typeNumber = if(type in 0..playerTypes.lastIndex) type else 0
    val checkStat = if(stat < 0 ) 10f else stat
    val member = "ㅇㅇ $name level $level, type ${playerTypes[typeNumber]}, stat $checkStat"


    //가입
    if(guild.containsKey(name)) println("$name ㅇㅇ 이미 가입되어 있습니다.") //닉네임 중복이 발생한다면
    else
...
/*fun joinToGuild(guild: MutableMap<String, String>, nick: String?) {
    val name = nick ?: "이름없음"
    val member = "ㅇㅇ예정자 $name level 1, type 검사, stat 1.0"

    //가입
    if(guild.containsKey(name)) println("$name ㅇㅇ예정자가 이미 가입되어 있습니다.") //닉네임 중복이 발생한다면
    else
        guild.put(name, member)
}
...
fun joinToGuild(guild: MutableMap<String, String>, nick: String?, level: Int) {
    val name = nick ?: "이름없음"
    val member = "ㅇㅇ예정자 $name level $level, type 검사, stat 1.0"
...
fun joinToGuild(guild: MutableMap<String, String>, nick: String?, level: Int, type: Int) {
    val playerTypes = arrayOf("직원", "검사", "마법사", "궁수", "성직자", "도적", "ㅇㅇ예정자", "ㅇㅇ자")
    val name = nick ?: "이름없음"
    val typeNumber = if(type in 0..playerTypes.lastIndex) type else 0
    val member = "ㅇㅇ예정자 $name level $level, type ${playerTypes[typeNumber]}, stat 1.0"
...
fun joinToGuild(guild: MutableMap<String, String>, nick: String?, level: Int, type: Int, stat: Float) {
    ...
    val member = "ㅇㅇ예정자 $name level $level, type ${playerTypes[typeNumber]}, stat $checkStat"
